...
- **All three techniques help, and complexity roughly tracks with payoff.** ``torch.compile()``
  ...
  up to **3.01x** on the 6-view multi-view transformer on an L4 (TensorRT FP16 + DALI vs. eager
  FP32).
...
.. figure:: https://i.imgur.com/99ytALk.png
...
(3.01x) on the 6-view multi-view transformer on L4 -- notably larger than the same technique's
...
substantially larger on A100 (44-53% slower) than on L4 (2-16% slower).
