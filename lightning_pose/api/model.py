...
class Model(_RuntimeMixin):  # pyright: ignore[reportGeneralTypeIssues]
    ...
    `Model`'s execution-backend methods (`compile()`, `export()`, and the
    eager/ONNX/TensorRT loading logic behind `from_dir(runtime=...)`) live in
    `_RuntimeMixin` (`lightning_pose/api/model_runtime.py`) and are mixed in here;
    this module owns construction and the `predict_*` methods.
...
def _build_datamodule_pred(cfg: DictConfig | ListConfig) -> BaseDataModule | UnlabeledDataModule:
    ...
    imgaug_transform_pred = get_imgaug_transform(cfg=cfg_pred, data_dir=cfg_pred.data.data_dir)
