...
class MultiviewFeatureTransformerHead(nn.Module):
    ...
    def forward(
        ...
        # tokens = [view * batch, num_features, rep_height, rep_width]

        # add positional embeddings to tokens
        tokens = self.add_positional_embeddings(tokens)
        # tokens = [view * batch, num_features + 1, rep_height, rep_width]

        # add view embeddings to tokens
        tokens = self.add_view_embeddings(tokens, num_views)
        # tokens = [view * batch, num_features + 2, rep_height, rep_width]

        # reshape tokens to feed into transformer
        # ...
        # tokens = [batch, view * rep_height * rep_width, num_features + 2]

        # feed tokens to transformer
        tokens = self.transformer(tokens)
        # tokens = [batch, view * rep_height * rep_width, num_features + 2]

        # reshape tokens to feed into heatmap head
        # ...
        # features = [view * batch, num_features + 2, rep_height, rep_width]

        # upsample features into heatmaps
        heatmaps = self.upsample(features)
f       # heatmaps = [view * batch, num_keypoints, heatmap_height, heatmap_width]

        # reshape heatmaps
        # ...
        # heatmaps = [batch, view * num_keyopints, heatmaps_height, heatmap_width]
...
#     """Transformer module that fuses information across multiple views."""
#     def __init__(
#         self,
#         in_channels: int,
#         intermediate_channels: int,
