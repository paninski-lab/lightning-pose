    ...
    if cfg.model.model_type == "regression":
        ...
        if cfg.data.get("view_names", None) and len(cfg.data.view_names) > 1:
            ...
            dataset = MultiviewHeatmapDataset(
                ...
                camera_params_path=cfg.data.get(
                    "camera_params_file", None
                ),  # No longer needed, derivable from (sessionkey, view)
                bbox_paths=cfg.data.get(
                    "bbox_file", None
                ),  # No longer needed: derivable from (labelfilekey, view)
            )
        else:
