...
def get_model(
    ...
    if "vit" in cfg.model.backbone or "beast3d" in cfg.model.backbone:
        if image_h != image_w:
    ...
    if not semi_supervised:
        ...
        elif cfg.model.model_type == "heatmap_multiview_aggregator":
            from lightning_pose.models import HeatmapTrackerMultiviewAggregator
            model = HeatmapTrackerMultiviewAggregator(
                num_keypoints=cfg.data.num_keypoints,
                num_views=len(cfg.data.view_names),
                loss_factory=loss_factories["supervised"],
                backbone=cfg.model.backbone,
        ...
        elif cfg.model.model_type == "heatmap_3d_transformer":
            from lightning_pose.models import HeatmapTracker3DTransformer
            model = HeatmapTracker3DTransformer(
                num_keypoints=cfg.data.num_keypoints,
                num_views=len(cfg.data.view_names),
                loss_factory=loss_factories["supervised"],
                backbone=cfg.model.backbone,
...
def get_callbacks(
    ...
    if (
        cfg.model.model_type == "heatmap_multiview_transformer" or cfg.model.model_type == "heatmap_multiview_aggregator"
        or cfg.model.model_type == "heatmap_3d_transformer"
