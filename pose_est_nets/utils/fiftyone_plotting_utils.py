...
def create_gt_dataset(cfg: DictConfig) -> None:
    csv_data = pd.read_csv(
        os.path.join(cfg.data.data_dir, cfg.data.csv_path), header=OmegaConf.to_object(cfg.data.header_rows)
    )
...
def evaluate(cfg, datamod, model, trainer):
    ...
    for idx, sample in enumerate(gt_dataset):
        ...
        resized_pred = reverse_transform(images=img.numpy(), keypoints=pred.numpy().reshape((datamod.test_batch_size, -1, 2)))[1]
        ...
        sample["prediction"] = fo.Keypoints(
            keypoints=[
                fo.Keypoint(
                    points = kpts_list
                )
