...
class HeatmapTracker(BaseFeatureExtractor):
    ...
    def __init__(
        ...
        self.output_sigma = torch.tensor(
            output_sigma,
            device=self.device
        )
        self.upsample_factor = torch.tensor(
            upsample_factor,
            device=self.device
        )
        self.confidence_scale = torch.tensor(
            confidence_scale,
            device=self.device
        )
...
class SemiSupervisedHeatmapTracker(HeatmapTracker):
    ...
    def __init__(
            ...
            threshold: Optional[float]=None,
            torch_seed: int=123,
            loss_params: Optional[dict]=None, # optional so we can initialize a model without passing that in
