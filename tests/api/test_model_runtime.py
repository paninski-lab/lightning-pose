...
from tests.api.model_helpers import _setup_test_model
...
def _tensorrt_available() -> bool:
    ...
    ``_tensorrt_version()`` in ``lightning_pose/api/model_runtime.py``, which uses
    the same signal to record the installed version in export metadata.
    """
    try:
...
def mock_ckpt():
    ...
    with patch(
        "lightning_pose.api.model_runtime.io_utils.ckpt_path_from_base_path",
