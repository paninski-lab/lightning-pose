...
def test_Multiview_heatmap_datamodule(cfg, Multiview_heatmap_data_module):
    ...
    assert batch["images"].shape == (train_size, 3, im_height * num_view, im_width)
    assert batch["keypoints"].shape == (train_size, num_targets)
    assert batch["heatmaps"].shape == (
        train_size,
        num_targets // 2,
        im_height_ds * num_view,
        im_width_ds,
    )
